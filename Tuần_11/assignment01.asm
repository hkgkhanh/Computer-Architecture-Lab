...
polling:
	li $t3, 0x01						# check row 1st with key 0, 1, 2, 3
	sb $t3, 0($t1)					# must reassign expected row
	lb $a0, 0($t2)					# read scan code of key button
	bne $a0, $zero, print			# if detect a key pressed then print
	
	li $t3, 0x02						# check row 2nd with key 4, 5, 6, 7
	sb $t3, 0($t1)					# must reassign expected row
	lb $a0, 0($t2)					# read scan code of key button
	bne $a0, $zero, print			# if detect a key pressed then print
	
	li $t3, 0x04						# check row 3rd with key 8, 9, A, B
	sb $t3, 0($t1)					# must reassign expected row
	lb $a0, 0($t2)					# read scan code of key button
	bne $a0, $zero, print			# if detect a key pressed then print
	
	li $t3, 0x08						# check row 4th with key C, D, E, F
	sb $t3, 0($t1)					# must reassign expected row
	lb $a0, 0($t2)					# read scan code of key button
	bne $a0, $zero, print			# if detect a key pressed then print
